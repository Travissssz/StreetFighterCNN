...
entity street_fighter_cnn_control_s_axi is
generic (
...
port (
    ...
    confidence            :in   STD_LOGIC_VECTOR(15 downto 0);
    dbg_in0               :in   STD_LOGIC_VECTOR(15 downto 0);
    dbg_l1_0              :in   STD_LOGIC_VECTOR(15 downto 0);
    dbg_gap0              :in   STD_LOGIC_VECTOR(15 downto 0);
    dbg_score0            :in   STD_LOGIC_VECTOR(15 downto 0);
...
end entity street_fighter_cnn_control_s_axi;
...
architecture behave of street_fighter_cnn_control_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    constant ADDR_CONFIDENCE_DATA_0 : INTEGER := 16#2c#;
    constant ADDR_CONFIDENCE_CTRL   : INTEGER := 16#30#;
    constant ADDR_DBG_IN0_DATA_0    : INTEGER := 16#3c#;
    constant ADDR_DBG_IN0_CTRL      : INTEGER := 16#40#;
    constant ADDR_DBG_L1_0_DATA_0   : INTEGER := 16#4c#;
    constant ADDR_DBG_L1_0_CTRL     : INTEGER := 16#50#;
    constant ADDR_DBG_GAP0_DATA_0   : INTEGER := 16#5c#;
    constant ADDR_DBG_GAP0_CTRL     : INTEGER := 16#60#;
    constant ADDR_DBG_SCORE0_DATA_0 : INTEGER := 16#6c#;
    constant ADDR_DBG_SCORE0_CTRL   : INTEGER := 16#70#;
    ...
    signal int_confidence      : UNSIGNED(15 downto 0) := (others => '0');
    signal int_dbg_in0         : UNSIGNED(15 downto 0) := (others => '0');
    signal int_dbg_l1_0        : UNSIGNED(15 downto 0) := (others => '0');
    signal int_dbg_gap0        : UNSIGNED(15 downto 0) := (others => '0');
    signal int_dbg_score0      : UNSIGNED(15 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CONFIDENCE_DATA_0 =>
                        rdata_data <= RESIZE(int_confidence(15 downto 0), 32);
                    when ADDR_DBG_IN0_DATA_0 =>
                        rdata_data <= RESIZE(int_dbg_in0(15 downto 0), 32);
                    when ADDR_DBG_L1_0_DATA_0 =>
                        rdata_data <= RESIZE(int_dbg_l1_0(15 downto 0), 32);
                    when ADDR_DBG_GAP0_DATA_0 =>
                        rdata_data <= RESIZE(int_dbg_gap0(15 downto 0), 32);
                    when ADDR_DBG_SCORE0_DATA_0 =>
                        rdata_data <= RESIZE(int_dbg_score0(15 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_confidence <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (ap_done = '1') then
                    int_confidence <= UNSIGNED(confidence);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_dbg_in0 <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (ap_done = '1') then
                    int_dbg_in0 <= UNSIGNED(dbg_in0);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_dbg_l1_0 <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (ap_done = '1') then
                    int_dbg_l1_0 <= UNSIGNED(dbg_l1_0);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_dbg_gap0 <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (ap_done = '1') then
                    int_dbg_gap0 <= UNSIGNED(dbg_gap0);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_dbg_score0 <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (ap_done = '1') then
                    int_dbg_score0 <= UNSIGNED(dbg_score0);
                end if;
            end if;
        end if;
    end process;
